  ...
  useEffect(() => {
    ...
    if (templateData) {
      try {
        ...
        form.setValue('scenarioCount', data.scenarioCount || 3);


        // Clear the template data after loading
        localStorage.removeItem('templateData');

        // Scroll to the form and focus on the title input
        setTimeout(() => {
                      ...
                      <FormControl>
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const currentStory = editForm.getValues("story");
                            if (currentContent) {
                              const updatedContent = updateFeatureContent(currentContent, e.target.value, currentStory);
...
function updateFeatureContent(content: string, newTitle: string, story?: string): string {
  ...
  // Add user story after Feature line if provided
  if (story) {
    ...
    if (featureLineIndex !== -1) {
      ...
      lines.splice(featureLineIndex + 1, 0, ...storyLines);
...
function ComplexityAnalysis({ featureId, autoAnalyze = false, feature, permissions }: { featureId: number, autoAnalyze?: boolean, feature?: Feature, permissions?: any }) {
  ...
  return (
    <div className="space-y-6">
      {/* Commented out Team Config and Development Planning Insights
