...
export default function Projects() {
  ...
  const deleteProjectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Project and its epics have been moved to unassigned.",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const deleteEpicMutation = useMutation({
    mutationFn: async (epicId: number) => {
      const res = await apiRequest("DELETE", `/api/epics/${epicId}`);
      if (!res.ok) {
        const error = await res.json();
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Epic Deleted",
        description: "Epic and its features have been moved to unassigned.",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const onDragEnd = (result: any) => {
    ...
    if (type === 'feature') {
      ...
      // Handle both "123" and "feature-123" formats
      const featureIdStr = draggableId.replace('feature-', '');
      const featureId = parseInt(featureIdStr);
      if (isNaN(featureId)) {
        console.error('Invalid feature ID:', draggableId);
        return;
      }
      ...
      if (destination.droppableId.startsWith('epic-')) {
        ...
        if (isNaN(newEpicId)) {
          console.error('Invalid epic ID in droppableId:', destination.droppableId);
          return;
        }
      ...
      // Handle both "123" and "epic-123" formats
      const epicIdStr = draggableId.replace('epic-', '');
      const epicId = parseInt(epicIdStr);
      if (isNaN(epicId)) {
        console.error('Invalid epic ID:', draggableId);
        return;
      }
      ...
      if (destination.droppableId.startsWith('project-')) {
        ...
        if (isNaN(newProjectId)) {
          console.error('Invalid project ID in droppableId:', destination.droppableId);
          return;
        }
  ...
  return (
    <>
      ...
      <main id="main-content" className="container mx-auto p-6 space-y-6" role="main">
        <header className="flex justify-between items-center">
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              <Badge variant="secondary" aria-label={`${(features as Feature[]).length} total features`}>
                <FileText className="w-3 h-3 mr-1" aria-hidden="true" />
                {(features as Feature[]).length} Total Features
              </Badge>
              <Badge variant="outline" aria-label={`${(epics as Epic[]).length} epics`}>
                <Target className="w-3 h-3 mr-1" aria-hidden="true" />
                {(epics as Epic[]).length} Epics
              </Badge>
              <Badge variant="outline" aria-label={`${(projects as Project[]).length} projects`}>
                <FolderOpen className="w-3 h-3 mr-1" aria-hidden="true" />
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <section className="space-y-6" aria-label="Project and epic management" role="region">
                ...
                <div className="flex items-center justify-between">
                  ...
                  >
                    <CollapsibleTrigger 
                      ...
                      aria-label={`${expandedProjects.has(project.id) ? 'Collapse' : 'Expand'} project ${project.name}`}
                    >
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Are you sure you want to delete "{project.name}"? All epics in this project will become unassigned. This action cannot be undone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                    ...
                    <CollapsibleContent className="mt-4" id={`project-${project.id}-content`}>
                      ...
                      <Droppable droppableId={`project-${project.id}`} type="epic">
                        {(provided, snapshot) => (
                          <div 
                            ...
                            role="region"
                            aria-label={`Epic drop zone for ${project.name} project`}
                            aria-describedby={`project-${project.id}-drop-instructions`}
                          >
                            <div id={`project-${project.id}-drop-instructions`} className="sr-only">
                              Drag and drop epics here. Use arrow keys to navigate, space to select, and arrow keys while selected to move.
                            </div>
                                          ...
                                          <AlertDialog>
                                            ...
                                            <AlertDialogContent>
                                              <AlertDialogHeader>
                                                ...
                                                <AlertDialogDescription>
                                                  Are you sure you want to delete "{epic.name}"? All features in this epic will become unassigned. This action cannot be undone.
                                                </AlertDialogDescription>
                                              </AlertDialogHeader>
                                              <AlertDialogFooter>
                                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                          ...
                                                          <Tooltip>
                                                            ...
                                                            <TooltipContent side="top" className="max-w-sm">
                                                              <div className="space-y-2">
                                                                ...
                                                                <p className="text-xs text-primary">Click to view scenarios</p>
                                                              </div>
                                                            </TooltipContent>
                                                          </Tooltip>
                                                        </TooltipProvider>
      ...
      <Dialog open={newEpicDialog} onOpenChange={setNewEpicDialog}>
        <DialogTrigger asChild>
          <Button disabled={(projects as Project[]).length === 0}>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            {selectedFeature?.generatedContent && (
              <div>
                <h4 className="text-sm font-medium mb-2">Generated Scenarios</h4>
            ...
            {selectedFeature?.generatedContent &&
              selectedFeature?.analysisJson && (
                <div className="mt-6 space-y-4">
                  ...
                  {(() => {
                    try {
                      const analysis = JSON.parse(selectedFeature.analysisJson);
                      const scenarioChunks = selectedFeature.generatedContent
                        .split("Scenario:")
                        .slice(1);
